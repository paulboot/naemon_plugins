...
# Updated : Frank Migge (support at frank4dd dot com)/Paul Boot
...
sub print_usage {
    print "Usage: $0 [-v] -H <host> -C <snmp_community> [-2] | (-l login -x passwd [-X pass -L <authp>,<privp>])  [-p <port>] -w <warn level> -c <crit level> [-t <timeout>] [-V]\n";
}
...
sub help {
   ...
   print "GPL licence, (c) 2007 Karl Bolingbroke, update (c)2010 Frank Migge/Paul Boot\n\n";
   print_usage();
   print <<EOT;
...
print " | offset=$offset;$o_warn;$o_crit";
print "\n";
